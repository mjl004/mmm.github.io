...
!function(a, b) {
	...
	function i(a) {
		var b = u.paths, c;
		return b && (c = a.match(G)) && w(b[c[1]]) && (a = b[c[1]] + c[2]),
		a
	}
	function j(a) {
		var b = u.vars;
	...
	function k(a) {
		...
		if (b)
			for (var d = 0, e = b.length; e > d; d++) {
				var f = b[d];
				if (c = y(f) ? f(a) || a : a.replace(f[0], f[1]),
				c !== a)
					break
			}
		return c
	...
	if (!a.seajs) {
		...
		s.prototype.load = function() {
			...
			if (!(a.status >= $.LOADING)) {
				...
				for (var d = a._remain = c.length, e, f = 0; d > f; f++)
					e = s.get(c[f]),
					e.status < $.LOADED ? e._waitings[a.uri] = (e._waitings[a.uri] || 0) + 1 : a._remain--;
				if (0 === a._remain)
					return a.onload(),
					b;
				var g = {};
				for (f = 0; d > f; f++)
					e = V[c[f]],
					e.status < $.FETCHING ? e.fetch(g) : e.status === $.SAVED && e.load();
				for (var h in g)
		...
		s.define = function(a, c, d) {
			...
			B("define", f),
			f.uri ? s.save(f.uri, f) : W = f
		}
		,
		s.save = function(a, b) {
			var c = s.get(a);
			c.status < $.SAVED && (c.id = b.id || a,
		...
		t.require = function(a) {
			...
			return b.status < $.EXECUTING && (b.onload(),
			b.exec()),
			b.exports
		},
		u.base = O,
		...
		t.config = function(a) {
			for (var b in a) {
				...
				if (d && v(d))
					...
					x(d) ? c = d.concat(c) : "base" === b && ("/" !== c.slice(-1) && (c += "/"),
					c = l(c)),
					u[b] = c
			}
			return B("config", a),
